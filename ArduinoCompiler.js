(function () {
    ...
    */
    ...
    /* 
        ...
        If $b or $c cant be found in the array, $a will be defined as string.
    ...
    */
    ...
    /*
        ...
        "passageboolean" will become true
    ...
    */
    const compileIfStatement = (statement) => {
        ...
        for (let i = 0; i < statement.conditionalActions.length; i++) {
            ifStatement += classifyStatement(null, statement.conditionalActions[i]);

        }
        ifStatement += `}`;
        return ifStatement;
    };
    const compileElseIfStatement = (statement) => {
        let ifStatement = `else if(${statement.condition}){`;
        for (let i = 0; i < statement.conditionalActions.length; i++) {
            ifStatement += classifyStatement(null, statement.conditionalActions[i]);
    ...
    */
    const compileElseStatement = (statement) => {
        ...
        for (let i = 0; i < statement.conditionalActions.length; i++) {
            elseStatement += classifyStatement(null, statement.conditionalActions[i]);
        }
        elseStatement += `}`;
    ...
    */
    const compileGotoStatement = (header, statement) => {
        return `passageBool${statement.value} = true;passageBool${header} = false;passage${statement.value}();`;
    };
    ...
    */
    ...
    const compilePassageText = (header, statement) => {
        ...
        if (variables) {
            for (let i = 0; i < variables.length; i++) {
                ...
                if (value) {
                    ...
                    statementText = statementText.replace(variables[i], " %d");
                    printf = "," + JSON.stringify(variables).replace("[", "").replace("]", "").replace(/\$/g, "").replace(/['"]+/g, '');
                }   
            }
        }
        return `tft.fillRoundRect(160, 200, 160, 40, 8, RED); 
                ...
                tft.printf("${statementText}"${printf});`;
    };

    /*
        Compiling link commands (buttons that user presses to navigate in ui)
    ...
    */
    ...
    const compileFaceStatement = (statement) => {
        return `${statement}();`;
    };
    /*
    ...
    */
    ...
    const classifyStatement = (header, passage) => {
        console.log("Compiling statement: " + JSON.stringify(passage, null, 2));
        switch (passage.statement) {
            ...
            case "face":
                return compileFaceStatement(passage);
            case "goto":
    ...
    */
    const compileArduinoCode = (tweeJSON) => {
        //console.log(JSON.stringify(tweeJSON, null, 2))
        let passageLinks = [];
        let arduinoCode = ``
    ...
    const insertToFile = (codeString) => {
        ...
        originalCode.splice(writeToLine, 0, codeString);
